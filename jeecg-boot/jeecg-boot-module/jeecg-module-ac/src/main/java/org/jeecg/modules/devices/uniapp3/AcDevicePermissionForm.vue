      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['useStatus']"
                  :label="get4Label('可用状态')"
                  name='useStatus'
                  prop='useStatus'
                  placeholder="请选择可用状态"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('AcDevicePermissionList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/devices/acDevicePermission/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/devices/acDevicePermission/edit':'/devices/acDevicePermission/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
