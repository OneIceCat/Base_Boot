...
layout: 'default',
style: {
navigationBarTitleText: '命令表',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="命令表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './AcCommandData';
defineOptions({
  name: 'AcCommandList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/devices/acCommand/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/devices/acCommand/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
