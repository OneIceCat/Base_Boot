...
const dataId = ref('')
const backRouteName = ref('AcDevicesList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/devices/acDevices/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/devices/acDevices/edit':'/devices/acDevices/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
