      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['senderNumber']"
                  :label="get4Label('发送号码')"
                  name='senderNumber'
                  prop='senderNumber'
                  placeholder="请选择发送号码"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('AcReceivedList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/devices/acReceived/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/devices/acReceived/edit':'/devices/acReceived/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
