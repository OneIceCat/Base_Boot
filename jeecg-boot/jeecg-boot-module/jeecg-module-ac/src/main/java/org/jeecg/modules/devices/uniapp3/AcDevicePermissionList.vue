...
layout: 'default',
style: {
navigationBarTitleText: '设备数据权限信息',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="设备数据权限信息"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './AcDevicePermissionData';
defineOptions({
  name: 'AcDevicePermissionList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/devices/acDevicePermission/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/devices/acDevicePermission/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
