...
<template>
  <PageLayout navTitle="短信列表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './AcReceivedData';
defineOptions({
  name: 'AcReceivedList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/devices/acReceived/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/devices/acReceived/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
