...
<template>
  <PageLayout navTitle="设备管理表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './AcDevicesData';
defineOptions({
  name: 'AcDevicesList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/devices/acDevices/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/devices/acDevices/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
